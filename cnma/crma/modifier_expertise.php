...
if(isset($_POST['modifier'])){
    ...
    if ($is_modal) {
        $refresh = "voir_dossier.php?id=".$id_dossier."&tab=expertise&updated=1";
...
<div class="crma-card" style="margin:0;">
    <h4><i class="fa fa-pen"></i> Modifier expertise</h4>
 <form method="POST"
      action="modifier_expertise.php?id=<?php echo $id; ?>">
        <div class="form-group">
            <label>Montant indemnité (DA)</label>
            <input type="number" step="1" onwheel="this.blur()" name="montant" value="<?php echo htmlspecialchars($expertise['montant_indemnite'] ?? '', ENT_QUOTES, 'UTF-8'); ?>" required>
        </div>
        <div class="form-group">
            <label>Commentaire</label>
            <input type="text" name="commentaire" value="<?php echo htmlspecialchars($expertise['commentaire'] ?? '', ENT_QUOTES, 'UTF-8'); ?>">
        </div>
        <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap;">
            <?php if ($is_modal): ?>
                <button type="button" class="btn btn-outline" data-vd-cancel>Annuler</button>
            <?php else: ?>
                <a class="btn btn-outline" href="voir_dossier.php?id=<?php echo $expertise['id_dossier']; ?>&tab=expertise">Retour</a>
            <?php endif; ?>
