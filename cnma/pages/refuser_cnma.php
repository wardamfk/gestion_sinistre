...
function notifyAssure($conn, $id_dossier, $id_expediteur, $type, $msg) {
    ...
    notifyAssure($conn, $id_dossier, $user_id, 'validation', "Bonne nouvelle ! Votre dossier $num a été validé par la CNMA. Votre agence CRMA vous contactera pour le règlement.");
}
...
if(isset($_GET['id'])) {
    ...
    mysqli_query($conn, "INSERT INTO historique
        ...
        VALUES ($id_dossier, 'Refus CNMA', NOW(), $user_id, $ancien_etat, $nouvel_etat,
                'Dossier refusé par la CNMA')");

    $msg = "Le dossier $num a été REFUSÉ par la CNMA. Veuillez contacter votre responsable.";
    mysqli_query($conn, "INSERT INTO notification
        (id_dossier, id_expediteur, id_destinataire, type, message)
        VALUES ($id_dossier, $user_id, $id_agent, 'refus', '$msg')");
